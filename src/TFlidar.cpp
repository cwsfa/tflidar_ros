...
namespace benewake{
    ...
    bool TFlidar::readData(unsigned char* _buf, int _nRead){ // Read data through serial communication
      int total = 0, ret = 0;
    ...
    float TFlidar::getDist(){ // Get distance from TFlidar
      while(true){
    ...
    void TFlidar::closePort(){ // Close serial port
      close(serial_);
    }
}